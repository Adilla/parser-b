...
let get_statistics (graph:t) : t_stats =
  let aux _ v st =
    match v with
    | Machine { imported=None; included=[]; _ } ->
...
let rec get_implementation (graph:t) (mch_name:string) : (string*Implementation.t) option =
  ...
  | Some Machine { refined=Some ref; _ }
  | Some Refinement { refined=Some ref; _ } -> get_implementation graph ref
...
let get_machines (graph:t) : string list =
  Hashtbl.fold (fun cname infos accu ->
      match infos with
      | Machine { imported=Some _; _ } | Machine { seen=_::_; _ } -> cname::accu
      | Machine { imported=None; seen=[]; refined=Some ref ; _} ->
       begin match get_implementation graph ref with
         | Some _ -> cname::accu
         | None ->( accu )
       end
      | Machine { imported=None; seen=[]; refined=None; included=_::_; _ } ->
        ( Printf.fprintf stdout "Skipping %s (included)\n" cname; accu )
      | Machine { imported=None; seen=[]; refined=None; included=[]; _ } ->
...
let get_sorted_machines (graph:t) : string list =
  ...
  and visit cname infos =
    match infos with
    | Machine { imported=Some _; sees ; _} | Machine { seen=_::_; sees; _ } ->
      let imports = match get_implementation graph cname with
        | None -> []
        | Some (_,imp) -> imp.imports
      in
      visit2 cname imports sees
    | Machine { imported=None; seen=[]; refined=Some ref; sees; _ } ->
      begin match get_implementation graph ref with
