...
let parse_component (graph:Graph.t) (filename:string) : unit =
  try
    ...
    | Ok c -> (close_in input; Graph.add_component graph c)
    | Error err -> (close_in input; print_error err)
  with
...
let _ =
  ...
  (if stats.missing_machines > 0 then 
     Printf.fprintf stdout "Number of missing machines: %i\n" stats.missing_machines);
