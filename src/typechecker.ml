...
let declare_operation_exn (cl:_ V.sclause) (env:_ Global.t) (op:P.operation) : _ T.operation =
  ...
  G.add_operation env op.P.op_name.lid_loc op.P.op_name.lid_str args_in args_out
    ~is_readonly:(is_read_only env (Local.get_vars ctx) op.P.op_body);
  ...
  | Some infos ->
    ...
    if Btype.is_equal (Global.get_alias env) te.T.exp_typ var_typ then
      let val_kind = match infos.Global.sy_kind with
        ...
        | G.Imp.Abstract_Set (S_Seen _|S_Imported _|S_Redeclared_In_Seen _|S_Redeclared_In_Imported _) ->
          Error.error v.lid_loc "This abstract set is not expected to be valued."
        | G.Imp.Concrete_Constant (C_Machine _|C_Refined|C_Redeclared_In_Machine _) -> T.VK_Concrete_Constant
        | G.Imp.Concrete_Constant (C_Seen _|C_Redeclared_In_Seen _
                                  |C_Redeclared_In_Imported _|C_Imported _) ->
        ...
        | G.Imp.Concrete_Set (_, _)
        | G.Imp.Enumerate _ ->
          Error.error v.lid_loc
...
let set_concretisation_exn f (env:_ Global.t) imports (v,e:lident*P.expression) : unit =
  ...
  | None -> () (*at this point, the concrete constants declared in the implementation are not in env *)
  | Some false -> ()
  | Some true ->
