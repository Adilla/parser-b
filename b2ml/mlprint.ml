...
let t_symb_to_string (ts:qident) : string =
  ...
  | None -> Codegen.ML_ident.to_string ts.q_id
  | Some ns -> (Codegen.ML_ident.pkg_to_string ns) ^ "." ^ (Codegen.ML_ident.to_string ts.q_id)  (* ???? *)
...
let b0_binary_op_to_string : t_b0_binary_op -> string = function
  ...
  | B0_Division -> "/"
  | B0_Modulo -> "mod"
  | B0_Power -> "pow" (** let say that for the moment **)
...
let rec mk_subst (s0:t_b0_subst) : Easy_format.t =
  ...
  | B0_Null -> Error.raise_exn lc "OCaml does not have a null value."
  ...
  | B0_IfThenElse (cases,def) ->
    ...
    let aux (p,s) =
      let elsifthen = mk_list "else if" "" "then" st [mk_expr p] in
  ...
  | B0_Case (e,cases,def) ->
    let st = Easy_format.list in
    let case = mk_list "match" "" "with" st [mk_expr e] in
    let aux (lst,s) =
...
let mk_type (ty:t_type) : Easy_format.t =
  ...
  | D_Alias ts ->
    mk_atom ("subtype " ^ Codegen.ML_ident.to_string ty.ty_name ^ " is "
             ^ t_symb_to_string ts ^ ";")
  | D_Int ->
...
let mk_const (c:t_constant_or_fun) : Easy_format.t =
  ...
  | Cst ({c_init=Init c_init; _} as c) ->
    let ty = mk_atom (Codegen.ML_ident.to_string c.c_name ^ ": constant "
...
let mk_var (v:t_variable) : Easy_format.t =
  ...
  | None ->
    mk_atom (Codegen.ML_ident.to_string v.v_name ^ ": "
...
let mk_proc_spec (p:t_procedure) : Easy_format.t =
  ...
  | Body _ ->
    begin match args with
      | [] -> mk_atom ("procedure " ^ Codegen.ML_ident.to_string p.p_name ^ ";")
      | _ ->
        let lb = mk_atom ("procedure " ^ Codegen.ML_ident.to_string p.p_name) in
        let st = { Easy_format.list with
                   Easy_format.space_after_opening = false;
                   stick_to_label = false;
                   space_before_separator = false;
...
let package_to_format_spec (pkg:t_package) : Easy_format.t =
  ...
  let st_body = { Easy_format.list with
                  ...
                  indent_body = 3 }
  in 
  let body = mk_list ("package "^Codegen.ML_ident.pkg_to_string pkg.pkg_name^" is") ""
      ("end "^Codegen.ML_ident.pkg_to_string pkg.pkg_name^";") st_body
...
let mk_proc_body (p:t_procedure) : Easy_format.t option =
  let pr = mk_atom ("procedure " ^ Codegen.ML_ident.to_string p.p_name) in
  let args = (List.map (mk_arg false) p.p_args_in)@(List.map (mk_arg true) p.p_args_out) in
  match p.p_body with
  | Body s ->
    let proc =
    ...
    in
    let body =
      mk_list "begin" "" ("end " ^ Codegen.ML_ident.to_string p.p_name ^ ";")
        st [mk_subst s]
    in
...
let package_to_format_body (pkg:t_package) : Easy_format.t option =
  ...
  | _ ->
    let st = { Easy_format.list with
               ...
               indent_body = 3 }
    in
    Some (mk_list ("package body "^Codegen.ML_ident.pkg_to_string pkg.pkg_name^" is") ""
            ("end "^Codegen.ML_ident.pkg_to_string pkg.pkg_name^";") st procs)
