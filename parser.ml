...
let rec loop_exn (state:state) (chkp:Component.u_comp I.checkpoint) : Component.u_comp =
  match chkp with
...
let parse_component_exn (filename:string) (input:in_channel) : Component.u_comp =
...
let parse_component (filename:string) (input:in_channel) : (Component.u_comp,loc*string) result =
  try Ok (parse_component_exn filename input)
  with Error.Error (p,msg) -> Error (p,msg)

let parse_component_from_string_exn (input:string) : Component.u_comp =
  let state = mk_state_from_string_exn input in
  loop_exn state (Grammar.Incremental.component_eof (get_current_pos state))

let parse_component_from_string (input:string) : (Component.u_comp,loc*string) result =
