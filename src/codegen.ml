...
let to_b0_constant = function
  ...
  | INTEGER | NATURAL | NATURAL1 | INT | NAT | Successor | Predecessor
...
*)
  ...
  | T.Builtin_2 (Application,ff,arg) ->
    begin match (try Some (to_b0_expr f ff) with _ -> None) with
      ...
      | None ->
        Error.raise_exn e.T.exp_loc "This is not a valid B0 array access."
(*
      ...
      | T.Builtin_2 (Couple _,e1,e2) ->
        begin match (to_b0_expr f e1).exp0_desc with
          | B0_Builtin_0 (B0_Integer j) ->
            if (Int64.of_int i) = j then (j,to_b0_expr f e2)
...
let rec to_b0_subst : 'mr 'cl. (('mr,'cl) V.t_global_ident -> t_full_ident_kind) ->
  ...
  | T.BecomesSuch _ -> Error.raise_exn stmt.T.sub_loc "This is not a valid B0-substitution (:()."
...
let get_imp_types (ref:t_id) (concrete_sets:((G.t_ref,G.t_concrete) T.symb*string list) list)
  ...
  let add_concrete_set lst (s,elts) : t_type list =
    ...
    | G.D_Redeclared G.By_Included_Or_Imported mch ->
      ...
      { ty_name; ty_def=D_Alias (mch,s.T.sy_id) }::lst
  in
  let add_abstract_set lst s : t_type list =
    match s.T.sy_src with
    | G.D_Machine loc | G.D_Redeclared G.By_Machine loc ->
...
let get_imp_variables (ref:t_id) (v:(G.t_ref,G.t_concrete) T.symb) : t_variable option =
  let aux v_name v_promoted_from =
    Some { v_name; v_promoted_from; v_type=(match to_b0_type v.T.sy_typ with
        | Some ty -> ty
        | None ->
          begin match v_name with
            | Intern lid ->
              Error.raise_exn lid.lid_loc
                ("The type of the variable '"^lid.lid_str^"' is '"^
                 Btype.to_string v.T.sy_typ^"'. This is not a valid B0-type.")
            | Extern (pkg,id) ->
  ...
  | G.D_Machine loc | G.D_Redeclared G.By_Machine loc ->
    aux (Intern (mk_lident loc v.T.sy_id)) None
  | G.D_Included_Or_Imported _ | G.D_Seen _ -> None
  | G.D_Redeclared G.By_Included_Or_Imported mch ->
    aux (Extern(mch,v.T.sy_id)) (Some mch)
  | G.D_Redeclared G.Implicitely ->
    aux (Extern(ref,v.T.sy_id)) None
...
let get_imp_operations (op:(Global.t_ref,V.t_imp_op) T.operation) : t_procedure =
    ...
    let to_arg (str,ty) =
      { arg_name=Extern(op_name, str);
        arg_type=(match to_b0_type ty with
            | Some ty -> ty
            | None ->
              Error.raise_exn op_name.lid_loc
                ("The argument '"^str^"' of this operation is not a valid B0-expression. Its type is '"
                 ^Btype.to_string ty^"'.") 
          ) }
    in
    { p_name = op_name;
  ...
  | T.O_Local { op_out; op_name; op_in; op_body; _ } ->
    let to_arg v =
      { arg_name=Intern(mk_lident v.T.arg_loc v.T.arg_id);
        arg_type=(match to_b0_type v.T.arg_typ with
            | Some ty -> ty
...
let get_imported_constants (sy:(G.t_ref,G.t_concrete) T.symb) : t_constant option =
  ...
  | G.D_Included_Or_Imported mch ->
    begin match to_b0_type sy.T.sy_typ with
      | None ->
        Error.raise_exn mch.lid_loc
          ("The constant '"^sy.T.sy_id^"' has type '" ^ Btype.to_string sy.T.sy_typ 
           ^"'. This is not a valid B0-expression.");
      | Some c_typ ->
...
let get_mch_variable (v:(G.t_mch,G.t_concrete) T.symb) : t_variable option =
  let aux v_name =
    ...
    | Some v_type -> Some { v_name; v_type; v_promoted_from=None }
  in
  match v.T.sy_src with
...
let get_mch_operation (op:(G.t_mch,V.t_mch_op) T.operation) : t_procedure =
  match op with
  | T.O_Specified { op_out; op_name; op_in; _ } ->
    let to_arg v =
      { arg_name=Intern(mk_lident v.T.arg_loc v.T.arg_id);
        arg_type=(match to_b0_type v.T.arg_typ with
            | Some ty -> ty
