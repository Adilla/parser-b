...
let rec sexp_of_expr : R.expression -> t = fun e ->
  ...
  | Comprehension (ids,p) ->
    let ids = List (List.map sexp_of_string (Nlist.to_list ids)) in
    List [Atom "Compr";ids;sexp_of_pred p]
  | Binder (bi,xlst,p,e) ->
    let ids = List (List.map sexp_of_string (Nlist.to_list xlst)) in
    List [Atom (binder_to_string bi);ids;sexp_of_pred p;sexp_of_expr e]
  | Record_Field_Access (e,id) ->
    List [Atom "Record_Field_Access";sexp_of_expr e;sexp_of_string id]
  | Record nlst ->
    let aux (id,e) = List [sexp_of_string id;sexp_of_expr e] in
    List ((Atom "Rec")::(List.map aux (Nlist.to_list nlst)))
  | Record_Type nlst ->
    let aux (id,e) = List [sexp_of_string id;sexp_of_expr e] in
    List ((Atom "Rec")::(List.map aux (Nlist.to_list nlst)))

and sexp_of_pred : R.predicate -> t = fun p ->
...
let rec sexp_of_subst : R.substitution -> t = fun s ->
  ...
  | Affectation (Function(f,elst),a) ->
    ...
    List [Atom "AFF2";sexp_of_string f;exprs;sexp_of_expr a]
  | Affectation (Record(id,fd),e) ->
    List [Atom "AFF3";sexp_of_string id;sexp_of_string fd;sexp_of_expr e]
  ...
  | CallUp (outs,f,args) ->
    List ( ((Atom "CALL")::(List.map sexp_of_string outs))@
          ((sexp_of_string f)::(List.map sexp_of_expr args)) )
  | While (p,s,q,e) ->
...
let sexp_of_set : _ set -> t = function
  | Abstract_Set v -> sexp_of_string v
  | Concrete_Set (v,lst) ->
    List[sexp_of_string v;List (List.map sexp_of_string lst)]

let sexp_of_minst (mi:_ machine_instanciation) : t =
  List [Atom mi.mi_mch; List (List.map sexp_of_expr mi.mi_params)]

let sexp_of_op (op:_ operation) : t =
  List [ List (List.map sexp_of_string op.op_in);
         sexp_of_string op.op_name;
         List (List.map sexp_of_string op.op_out);
         sexp_of_subst op.op_body ]

let sexp_of_clause : R.clause -> t = fun c ->
  ...
  | Values nlst -> let aux (id,e) = List [sexp_of_string id;sexp_of_expr e] in
    List ( (Atom "VALUES")::(List.map aux (Nlist.to_list nlst)) )
  | Sees nlst -> List ( (Atom "SEES")::(List.map sexp_of_string (Nlist.to_list nlst)) )
  | Promotes nlst -> List ( (Atom "PROMOTES")::(List.map sexp_of_string (Nlist.to_list nlst)) )
  | Uses nlst -> List ( (Atom "USES")::(List.map sexp_of_string (Nlist.to_list nlst)) )
  | Sets nlst -> List ( (Atom "SETS")::(List.map sexp_of_set (Nlist.to_list nlst)) )
  | Constants nlst -> List ( (Atom "CONSTANTS")::(List.map sexp_of_string (Nlist.to_list nlst)) )
  | Abstract_constants nlst -> List ( (Atom "ABSTRACT_CONSTANTS")::(List.map sexp_of_string (Nlist.to_list nlst)) )
  | Concrete_variables nlst -> List ( (Atom "CONCRETE_VARIABLES")::(List.map sexp_of_string (Nlist.to_list nlst)) )
  | Variables nlst -> List ( (Atom "VARIABLES")::(List.map sexp_of_string (Nlist.to_list nlst)) )

let sexp_of_mch name parameters clauses : t =
  List ( (Atom "MACHINE")::(Atom ("ident_" ^ name))::
         (List (List.map sexp_of_string parameters))::
         (List.map sexp_of_clause clauses) )

let sexp_of_ref name refines parameters clauses : t =
  List ( (Atom "REFINEMENT")::(Atom ("ident_" ^ name))::
         (List (List.map sexp_of_string parameters))::
         (List [Atom "REFINES";sexp_of_string refines])::
         (List.map sexp_of_clause clauses) )
...
let sexp_of_component co =
  match co.R.co_desc with
  | Machine mch -> sexp_of_mch co.co_name co.co_parameters (R.get_clauses co)
  | Refinement ref -> sexp_of_ref co.co_name ref.ref_refines co.co_parameters (R.get_clauses co)
  | Implementation imp -> sexp_of_imp co.co_name imp.imp_refines co.co_parameters (R.get_clauses co)
